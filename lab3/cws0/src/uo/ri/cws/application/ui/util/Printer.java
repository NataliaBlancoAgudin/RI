...
public class Printer {
	...
	Console.printf("\t%s \t%-40.40s \t%td/%<tm/%<tY \t%-12.12s \t%.2f\n",
	    rep.id, rep.description, rep.date, rep.state, rep.amount);
    }
	...
	Console.printf("\t%s \t%-12.12s \t%.2f \t%.2f \t%.2f\n", p.contractId,
	    p.date, p.grossSalary, p.totalDeductions, p.netSalary);
    }
	...
	Console.printf("Mechanic: %s %s (NIF: %s)\n", c.mechanic.name,
	    c.mechanic.surname, c.mechanic.nif);
	...
	Console.printf(
	    ...
	    c.contractType.name, c.startDate,
